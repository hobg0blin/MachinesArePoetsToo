...
function setup() {
  ...
  let genSeed = select("#generate1");
  let genButton = select("#generate");
  let genResults = select("#results4")
  ...
  genButton.mousePressed(() => {
    let initSeed = genSeed.value();
    let output = '';
    genText(initSeed, output);
  });

  async function genText(seed, output) {
    for (let i = 0; i < 20; i++) {
      if (i % 2) {
        let nearest = await word2Vec.nearest(seed);
        let rand = int(random(nearest.length - 1));
        if (nearest) {
          output += nearest[rand].word + ' </br>';
          seed = nearest[0].word;
        } else {
          genResults.html('no vector found!')
        }
        console.log('seed: ', seed);
        console.log('output: ', output);
        genResults.html(output);

      } else {
        console.log('getting rndom word')
        rand = await word2Vec.getRandomWord()
        seed = rand;
        output += rand + '</br>';
      }
      genResults.html(output);
